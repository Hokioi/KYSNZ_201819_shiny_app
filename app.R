...
ui <- fluidPage(theme = shinytheme("lumen"),
        tabsetPanel(
            ...
            tabPanel("Pill testing results",
                ...
                sidebarLayout(
                    sidebarPanel(
                        helpText("KnowYourStuffNZ tested 805 pills at 13 events in 2018-19. 
                                 ...
                                 The chart and table show you what those pills actually contained."),
                ...
                mainPanel(
                    verbatimTextOutput("multiheading"),
                    br(),
            ...
            tabPanel("Instructions",
                mainPanel(
                    ...
                    p(a("KnowYourStuffNZ", href = "https://knowyourstuff.nz"),
                      ...
                      " to provide drug related harm reduction services at events around New Zealand. These pages show
                      KnowYourStuffNZ pill testing results for events attended between 1 April 2018 and 
                      31 March 2019."),
                    h3("How to see the results "),
                    p("Click on the 'Pill testing results' tab above."),
                    h4("Select which presumed substance/s you want to look at"),
                    p("In the panel of checkboxes, select at least one substance from the list. These substances are 
                    what clients thought they had before their pills had been tested."),
                    p("Once you have selected at least one substance, a chart and a table will appear. 
                    These show the testing results, i.e. what the pills (that clients thought contained the substance 
                    ...
                    p("If you select more than one presumed substance, the testing results will be added up across 
...
server <- function(input, output) {
        ...
        output$multiheading <-renderText ({
            
        #check if more than one presumed substance selected and create heading if so
            if(length(input$presumed) == 2) {return(paste("Results summed across both substances that you selected"))}
            if(length(input$presumed) > 2) {return(paste("Results summed across all substances that you selected"))}
        })
